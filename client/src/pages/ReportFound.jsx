...
const ReportFound = () => {
    ...
    const [form, setForm] = useState({ itemName: '', description: '', location: '', date: '', handedOverTo: '', contactInfo: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await lostFoundAPI.reportFound(fd);
    ...
    return (
        <div className="pt-20 pb-12 min-h-screen bg-gray-50">
            <div className="max-w-lg mx-auto px-4 py-8">
                ...
                <h1 className="text-2xl font-black text-gray-900 mb-6">🎉 Report Found Item</h1>
                <form onSubmit={handleSubmit} className="card p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" value={form.description} onChange={handleChange} className="input resize-none h-24" placeholder="Describe the item..." />
                    </div>
                    <div>
                        <label className="label">Where did you find it? *</label>
                        <input name="location" value={form.location} onChange={handleChange} className="input" placeholder="e.g. Ground floor corridor" />
                    </div>
                    <div>
                    ...
                    <div>
                        <label className="label">Handed Over To (optional)</label>
                        <input name="handedOverTo" value={form.handedOverTo} onChange={handleChange} className="input" placeholder="e.g. Hostel warden, Security desk" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        {images.length > 0 && (
                            <div className="flex gap-2 mt-2">
                                {images.map((img, i) => <img key={i} src={img.url} className="w-16 h-16 object-cover rounded-lg" />)}
                            </div>
                        )}
                    </div>
